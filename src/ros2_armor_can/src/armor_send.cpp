...
public:
    ...
    void send_motor_cmd(int16_t motor1, int16_t motor2, int16_t motor3, int16_t motor4) {
        ...
        RCLCPP_INFO(this->get_logger(), "准备发送自瞄角度: yaw=%d, pitch=%d, flag=%d", motor1, motor2, motor3);
...
private:
    ...
    void target_info_callback(const std_msgs::msg::Float32MultiArray::SharedPtr msg) {
        if (msg->data.size() >= 7) {
            double target_yaw = msg->data[4];
            double target_pitch = msg->data[5];
            double flag = msg->data[6];
            RCLCPP_INFO(this->get_logger(),
                "目标:(%.1f,%.1f) 光心:(%.1f,%.1f) yaw:%.2f pitch:%.2f flag:%.lf",
                msg->data[0], msg->data[1], msg->data[2], msg->data[3], target_yaw, target_pitch, flag);
            // 异步发送，避免阻塞回调线程
            std::thread([this, target_yaw, target_pitch, flag]() {
                this->send_motor_cmd(
                    static_cast<int16_t>(target_yaw),
                    static_cast<int16_t>(target_pitch),
