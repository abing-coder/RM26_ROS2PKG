        ...
        try {
           ...
           detectionArmor_.start_detection(image);
            ...
            if (!armors.empty()) {
                ...
                target_yaw = static_cast<float>(target.target_yaw);
                target_pitch = static_cast<float>(target.target_pitch);
            ...
            RCLCPP_INFO(this->get_logger(),
                "目标: (%.1f, %.1f) | 光心: (%.1f, %.1f) | yaw: %.2f | pitch: %.2f | 标志: %.1f",
                target_x, target_y, optical_center_x, optical_center_y, target_yaw, target_pitch, flag);
